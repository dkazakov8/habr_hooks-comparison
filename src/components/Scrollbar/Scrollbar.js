...
function renderTrackHorizontalDefault() {
...
function renderTrackVerticalDefault() {
...
export class Scrollbar extends Component {
  ...
  getThumbVerticalHeight() {
    ...
    if (trackHeight === height || (trackHeight === 0 && isNaN(height))) return 0;
    if (thumbSize) return thumbSize;
  ...
  setupDragging() {
    ...
    document.onselectstart = _.stubFalse;
  }
  ...
  update(callback) {
    this.requestAnimationFrame(() => this._update(callback));
  }

  _update(callback) {
    const { onUpdate, hideTracksWhenNotNeeded, noScrollClassName } = this.props;
    ...
    if (getScrollbarWidth()) {
      ...
      const showHorizontalScroll = scrollWidth > clientWidth;
      const showVerticalScroll = scrollHeight > clientHeight;
      const disableViewStyle = scrollWidth === clientWidth && scrollHeight === clientHeight;

      if (noScrollClassName) {
        if (disableViewStyle) {
          this.view.classList.add(noScrollClassName);
        } else {
          this.view.classList.remove(noScrollClassName);
        }
      }

      if (hideTracksWhenNotNeeded) {
        const trackHorizontalStyle = {
          visibility: showHorizontalScroll ? 'visible' : 'hidden',
          zIndex: showHorizontalScroll ? 1 : -1,
        };
        const trackVerticalStyle = {
          visibility: showVerticalScroll ? 'visible' : 'hidden',
          zIndex: showVerticalScroll ? 1 : -1,
        };
  ...
  render() {
    ...
    const viewStyle = {
      ...
      marginBottom: scrollbarWidth ? -scrollbarWidth - 15 : 0,
      ...(autoHeight && {
        ...viewStyleAutoHeight,
        // Add scrollbarWidth to autoHeight in order to compensate negative margins
        minHeight: _.isString(autoHeightMin)
          ? `calc(${autoHeightMin} + ${scrollbarWidth}px)`
          : autoHeightMin + scrollbarWidth,
        maxHeight: _.isString(autoHeightMax)
